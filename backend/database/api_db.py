...
def inicializar_dados() -> None:
    ...
    filmes_iniciais = [
        {'id': 1, 'titulo': 'Crepúsculo', 'diretor': 'Catherine Hardwicke', 'ano': 2005, 'nota': 5.3, 'criado_em': datetime.now()},
        {'id': 2, 'titulo': 'A Viagem de Chihiro', 'diretor': 'Hayao Miyazaki', 'ano': 2001, 'nota': 8.6, 'criado_em': datetime.now()},
        {'id': 3, 'titulo': 'Orgulho e Preconceito', 'diretor': 'Joe Wright', 'ano': 2005, 'nota': 7.8, 'criado_em': datetime.now()},
    ]
...
def adicionar_filme(filme: Dict) -> Dict:
    global _contador_id
    novo_filme = {'id': _contador_id, **filme, 'criado_em': datetime.now()}
    _api_db.append(novo_filme)
    _contador_id += 1  # Correção: incrementa o contador
    return novo_filme
